  ...
  return (
    <section id="about" className="relative py-24 sm:py-32 section-glow">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
              ...
              <Reveal key={c.letter} delay={i * 0.12}>
                <div className="glass rounded-2xl p-7 h-full transition-transform duration-300 hover:-translate-y-1.5">
                  <div className="flex items-center gap-3 mb-5">
                    <span className="grid place-items-center w-12 h-12 rounded-xl bg-gradient-to-br from-violet-electric/30 to-violet-glow/20 border border-violet-electric/30 text-violet-glow font-extrabold">
                      {c.letter[0]}
                    </span>
                  ...
                  <p className="text-[#b9b2d6] leading-relaxed">{c.desc}</p>
                  <Icon
                    size={20}
                    className="text-violet-glow/60 mt-5"
                    aria-hidden
                  />
                </div>
              </Reveal>
        ...
        {/* Vision */}
        <Reveal delay={0.1}>
          <div className="mt-16 rounded-3xl border border-violet-electric/25 bg-gradient-to-br from-violet-electric/10 to-transparent p-8 sm:p-12">
            <div className="flex items-center gap-3 mb-4">
              <Eye className="text-violet-glow" size={22} />
              <h3 className="text-sm font-semibold tracking-[0.25em] text-violet-glow uppercase">
                Our Vision
              </h3>
            </div>
            <p className="text-2xl sm:text-3xl font-bold text-white leading-snug max-w-4xl">
        ...
        <Reveal delay={0.15}>
          <div className="mt-10 rounded-3xl border border-white/10 bg-white/[0.03] p-8 sm:p-12">
            <div className="flex items-center gap-3 mb-6">
              <Target className="text-violet-glow" size={22} />
              <h3 className="text-sm font-semibold tracking-[0.25em] text-violet-glow uppercase">
                Our Mission
              </h3>
            </div>
            <ul className="space-y-4">
              {mission.map((m, i) => (
                <li key={i} className="flex gap-4 items-start">
                  <span className="mt-1.5 grid place-items-center w-6 h-6 rounded-full bg-violet-electric/20 border border-violet-glow/40 text-violet-glow text-xs font-bold shrink-0">
                    {i + 1}
                  </span>
                  <span className="text-lg text-[#d6d0ec] leading-relaxed">
                    {m}
                  </span>
                </li>
              ))}
